...
namespace GdTool {
    public class GdcToken {
        ...
        {
            return $"{Line}:{Column} {Type} {Operand}";
        }
        ...
        public DecompileBuffer Decompile(DecompileBuffer buf, GdcTokenType previous, BytecodeProvider provider) {
            switch (Type) {
                ...
                case GdcTokenType.AmpersandAmpersand:
                    return buf.AppendOp("&&");
                case GdcTokenType.PipePipe:
                    return buf.AppendOp("||");
                case GdcTokenType.Bang:
                    return buf.AppendOp("!");
                case GdcTokenType.StarStar:
                    return buf.AppendOp("**");
                case GdcTokenType.StarStarEqual:
                    return buf.AppendOp("**=");
                case GdcTokenType.CfWhen:
                    return buf.Append("when ");
                case GdcTokenType.PrAwait:
                    return buf.Append("await ");
                case GdcTokenType.PrNamespace:
                    return buf.Append("namespace ");
                case GdcTokenType.PrSuper:
                    return buf.Append("super");
                case GdcTokenType.PrTrait:
                    return buf.Append("trait ");
                case GdcTokenType.PeriodPeriod:
                    return buf.AppendOp("..");
                case GdcTokenType.Underscore:
                    return buf.Append("_");
                case GdcTokenType.Indent:
                    return buf;
                case GdcTokenType.Dedent:
                    return buf;
